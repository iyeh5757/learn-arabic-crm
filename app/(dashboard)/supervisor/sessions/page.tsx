...
export default function SupervisorSessionsPage() {
  ...
  useEffect(() => {
    // Load only teachers assigned to this supervisor
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '12px' }}>
        <div>
          ...
          <p style={{ color: '#6B7280', fontSize: '14px', margin: '4px 0 0 0' }}>Your teachers' sessions</p>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F9FAFB' }}>
                {['Date', 'Student', 'Teacher', 'Type', 'Duration', 'Attendance', 'Trial Status', 'Rating', 'Actions'].map(h => (
            ...
            <tbody>
              {loading && <tr><td colSpan={9} style={{ textAlign: 'center', padding: '48px', color: '#9CA3AF' }}>Loading…</td></tr>}
              {!loading && sessions.length === 0 && <tr><td colSpan={9} style={{ textAlign: 'center', padding: '48px', color: '#9CA3AF' }}>No sessions found</td></tr>}
              {sessions.map((s: any) => {
