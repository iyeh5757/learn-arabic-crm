...
function NewPaymentPageInner() {
  ...
  async function handleStudentChange(id: string, list: any[] = students) {
    const s = list.find(x => x.id === id)
    setForm(f => ({ ...f, student_id: id, currency: s?.currency ?? f.currency, payment_method: s?.payment_method ?? f.payment_method }))

    // Pull the student's most recent payment to prefill plan/amount/method
    const { data: lastPayment } = await supabase
      .from('payments')
      .select('number_of_classes, amount, currency, payment_method')
      .eq('student_id', id)
    ...
    if (lastPayment) {
      setForm(f => ({
        ...f,
        number_of_classes: lastPayment.number_of_classes ?? f.number_of_classes,
        amount: lastPayment.amount ?? f.amount,
        currency: lastPayment.currency ?? f.currency,
        payment_method: lastPayment.payment_method ?? f.payment_method,
        is_renewal: true,
      }))
    }
  }
