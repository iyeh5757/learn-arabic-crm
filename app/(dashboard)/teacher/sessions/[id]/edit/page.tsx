'use client'
// app/(dashboard)/teacher/sessions/[id]/edit/page.tsx
import { useState, useEffect } from 'react'
import { createClient } from '@/lib/supabase/client'
import { useRouter, useParams } from 'next/navigation'

export default function TeacherSessionEditPage() {
  ...
  useEffect(() => {
    supabase.from('sessions').select('*, student:students(name)').eq('id', id).single().then(({ data }) => {
  ...
  return (
    <div style={{ maxWidth: '860px' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#6B7280', fontSize: '14px', margin: '4px 0 0 0' }}>Student: {form.student?.name}</p>
      </div>
