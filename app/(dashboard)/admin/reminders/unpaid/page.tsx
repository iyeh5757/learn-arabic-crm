// app/(dashboard)/admin/reminders/unpaid/page.tsx
// First-Payment Follow-ups: customers who haven't made their first payment yet
// (trial not converted / payment pending). Kept separate from the renewals page.
import { createClient } from '@/lib/supabase/server'
import Link from 'next/link'
import { MarkInactiveButton } from '../RetentionActions'
...
export default async function AdminUnpaidPage() {
  ...
  const { data: unpaid } = await supabase
    .from('students')
    .select('id, name, phone, email, country, currency, student_status, created_at, notes, assigned_teacher:teachers(profile:profiles!teachers_user_id_fkey(name)), added_by_sales:profiles!students_added_by_sales_id_fkey(name)')
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead>
            <tr style={{ background: '#F9FAFB' }}>
              {['Student', 'Added', 'Teacher', 'Sales Agent', 'Phone', 'Country', 'Actions'].map(h => (
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '22px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '10px' }}>
        <div>
          ...
          <p style={{ color: '#6B7280', fontSize: '14px', margin: '4px 0 0' }}>Customers who haven&apos;t made their first payment — separate from renewal reminders</p>
        </div>
        <Link href="/admin/reminders" style={{ background: '#F1F5F9', color: '#334155', padding: '9px 16px', borderRadius: '9px', textDecoration: 'none', fontSize: '13px', fontWeight: 600 }}>← Back to Reminders</Link>
      </div>

      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: '14px' }}>
        {[
          { label: 'Trials not converted', value: trials.length, color: '#C2410C', bg: '#FFF7ED' },
          { label: 'Other unpaid', value: others.length, color: '#7C3AED', bg: '#F5F3FF' },
          { label: 'Total', value: (unpaid ?? []).length, color: '#0F172A', bg: '#F8FAFC' },
        ].map(k => (
