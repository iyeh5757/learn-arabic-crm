'use client'
// app/(dashboard)/admin/payments/[id]/edit/page.tsx
  ...
  return (
    <div style={{ maxWidth: '860px' }}>
      <div style={{ marginBottom: '24px' }}>
        ...
        <p style={{ color: '#6B7280', fontSize: '14px', margin: '4px 0 0 0' }}>Student: {studentInfo?.name}</p>
      </div>
      ...
      {studentInfo && (
        <div style={{ background: '#F9FAFB', border: '1px solid #E5E7EB', borderRadius: '12px', padding: '16px 20px', marginBottom: '20px', display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
          <div><p style={{ fontSize: '11px', color: '#6B7280', margin: '0 0 2px 0', textTransform: 'uppercase' }}>Total Classes</p><p style={{ fontWeight: '700', color: '#111827', margin: 0 }}>{studentInfo.total_paid_classes}</p></div>
          <div><p style={{ fontSize: '11px', color: '#6B7280', margin: '0 0 2px 0', textTransform: 'uppercase' }}>Consumed</p><p style={{ fontWeight: '700', color: '#111827', margin: 0 }}>{studentInfo.consumed_classes}</p></div>
          <div><p style={{ fontSize: '11px', color: '#6B7280', margin: '0 0 2px 0', textTransform: 'uppercase' }}>Remaining</p><p style={{ fontWeight: '700', color: (studentInfo.total_paid_classes - studentInfo.consumed_classes) <= 2 ? '#D97706' : '#059669', margin: 0 }}>{studentInfo.total_paid_classes - studentInfo.consumed_classes}</p></div>
        </div>
      )}
          ...
          <div style={grid}>
            <div><label style={lbl}>Number of Classes</label>
              <select style={inp} value={form.number_of_classes} onChange={e => setForm((f: any) => ({...f, number_of_classes: Number(e.target.value)}))}>
                ...
                <option value={form.number_of_classes}>{form.number_of_classes} sessions (current)</option>
              </select>
            </div>
            ...
            <div style={{ gridColumn: '1 / -1' }}><label style={lbl}>Notes</label>
              <textarea style={{ ...inp, minHeight: '70px', resize: 'vertical' }} value={form.notes ?? ''} onChange={e => setForm((f: any) => ({...f, notes: e.target.value}))} />
            </div>
          </div>
        </div>
