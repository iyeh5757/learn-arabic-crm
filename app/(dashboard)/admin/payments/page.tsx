...
export default async function PaymentsPage() {
  ...
  const { data: payments } = await supabase
    .from('payments')
    .select(`
      id, amount, currency, number_of_classes, payment_method,
      status, payment_date, is_renewal, created_at,
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div>
          ...
          <p style={{ color: '#6B7280', fontSize: '14px', margin: '4px 0 0 0' }}>{payments?.length ?? 0} total records</p>
        </div>
        <Link href="/admin/payments/new" style={{ background: '#0D1B2A', color: '#E8C97A', padding: '10px 20px', borderRadius: '10px', textDecoration: 'none', fontWeight: '600', fontSize: '14px' }}>
          + Add Payment
        </Link>
      </div>
      ...
      {Object.keys(totals).length > 0 && (
        <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap' }}>
          {Object.entries(totals).map(([cur, amt]) => (
            <div key={cur} style={{ background: '#0D1B2A', borderRadius: '12px', padding: '14px 20px', minWidth: '140px' }}>
              <p style={{ color: '#9CA3AF', fontSize: '11px', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '0 0 4px 0' }}>Total Paid ({cur})</p>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F9FAFB' }}>
                {['Date', 'Student', 'Classes', 'Amount', 'Method', 'Status', 'Type', 'Added By', 'Actions'].map(h => (
            ...
            <tbody>
              {(payments ?? []).length === 0 && (
                <tr><td colSpan={9} style={{ textAlign: 'center', padding: '48px', color: '#9CA3AF' }}>No payments yet.</td></tr>
              )}
              {(payments ?? []).map((p: any) => {
                ...
                return (
                  <tr key={p.id} style={{ borderBottom: '1px solid #F3F4F6' }}>
                    <td style={{ padding: '12px 16px', fontSize: '13px', color: '#374151', whiteSpace: 'nowrap' }}>
                      {p.payment_date ? new Date(p.payment_date).toLocaleDateString('en-GB') : new Date(p.created_at).toLocaleDateString('en-GB')}
                    </td>
                    ...
                    <td style={{ padding: '12px 16px', fontWeight: '700', color: '#111827', whiteSpace: 'nowrap' }}>{sym}{Number(p.amount).toLocaleString('en', { minimumFractionDigits: 2 })}</td>
                    ...
                    <td style={{ padding: '12px 16px' }}>
                      {p.is_renewal && <span style={{ background: '#EFF6FF', color: '#2563EB', padding: '3px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: '600' }}>Renewal</span>}
                    </td>
                    <td style={{ padding: '12px 16px', color: '#9CA3AF', fontSize: '12px' }}>{p.added_by_profile?.name ?? '—'}</td>
                    <td style={{ padding: '12px 16px' }}>
                      <Link href={`/admin/payments/${p.id}/edit`} style={{ background: '#0D1B2A', color: '#E8C97A', padding: '5px 12px', borderRadius: '6px', textDecoration: 'none', fontSize: '12px', fontWeight: '600' }}>Edit</Link>
                    </td>
                  </tr>
                )
              })}
