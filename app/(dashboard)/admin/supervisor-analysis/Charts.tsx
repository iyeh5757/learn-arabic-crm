...
export default function Charts({ byCountry, bySales }: { byCountry: Country[]; bySales: Sales[] }) {
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '16px' }}>
      <div style={cardBox}>
        <div style={title}>💰 Revenue by country (USD)</div>
      ...
      <div style={cardBox}>
        <div style={title}>💵 Revenue by sales rep (USD)</div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
              <thead>
                <tr style={{ background: '#F8FAFC' }}>
                  {['Sales rep', 'Students', 'Renewal rate', 'Revenue (USD)'].map(h => (
              ...
              <tbody>
                {bySales.map(s => (
                  <tr key={s.name} style={{ borderBottom: '1px solid #F3F4F6' }}>
                    ...
                    <td style={{ padding: '8px 12px', color: '#374151' }}>{usd(s.revenue)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
