...
import { getGroupSubject } from '@/lib/notifications/whatsapp'
...
export async function POST(req: Request) {
  ...
  for (const data of items) {
    ...
    const { data: existing } = await supabase.from('wa_conversations').select('id, student_id, name, wa_name').eq('wa_jid', jid).maybeSingle()

    let conversationId = existing?.id as string | undefined

    if (!conversationId) {
      // The WhatsApp name: group subject for groups, contact push name for people.
      const waName: string | null = isGroup ? await getGroupSubject(jid) : pushName
      // Try to match an incoming private number to a student
      let studentId: string | null = null
      let country: string | null = null
      let name: string | null = waName ?? (isGroup ? 'Group' : null)
      if (!isGroup && phoneDigits) {
        ...
        if (m) { studentId = m.id; country = m.country ?? null; if (!name) name = m.name ?? null }
      }
      const { data: created } = await supabase.from('wa_conversations').insert({
        wa_jid: jid, phone: isGroup ? null : phoneDigits, is_group: isGroup,
        name, wa_name: waName, student_id: studentId, country,
        last_message_at: ts, last_message_preview: body.slice(0, 120), last_direction: direction,
        unread_count: direction === 'in' ? 1 : 0,
      }).select('id').single()
      conversationId = created?.id
    } else {
      // Update conversation summary + keep the WhatsApp name fresh
      const upd: Record<string, any> = {
        last_message_at: ts, last_message_preview: body.slice(0, 120), last_direction: direction,
      }
      if (!isGroup && pushName) upd.wa_name = pushName
      if (isGroup && !existing?.wa_name) { const subj = await getGroupSubject(jid); if (subj) { upd.wa_name = subj; if (!existing?.name || existing.name === 'Group') upd.name = subj } }
      if (!existing?.name && (upd.wa_name || pushName)) upd.name = upd.wa_name ?? pushName
